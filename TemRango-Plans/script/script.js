const more = document.getElementsByClassName("more__title")
...
Array.from(more).forEach(item=>{
	item.addEventListener("click", ()=>{
		if(item.innerText == "ver mais"){
			item.innerText = "ver menos"
			item.parentElement.parentElement.style.border = "1px solid #05CB95"
			item.style.color = "#05CB95"
			item.parentElement.parentElement.style.maxHeight = "800px"
			Array.from(item.parentElement.children).forEach(item=>{
				if(item.className != "more__title"){
			...
			item.innerText = "ver mais"
			item.parentElement.parentElement.style.border = ""
			item.style.color = ""
			item.parentElement.parentElement.style.maxHeight = ""
			Array.from(item.parentElement.children).forEach(item=>{
				if(item.className != "more__title"){
...
function changeMenu(){
	if(menu[0].style.display == "block"){
		...
		mask[0].style.visibility = "visible"
		container[0].style.position = "fixed"
		container[0].style.marginLeft = "89px"
		mask[0].addEventListener('click', ()=>{
			anima(1,0,0)
			menu[0].style.display = "none"
			mask[0].style.visibility = "hidden"
